...
**wacrm** — a self-hostable CRM template for the official WhatsApp Business
API (Meta Cloud API): shared inbox, contacts, sales pipelines, broadcasts,
no-code automations/flows, and an AI reply assistant. Next.js 16 (App
Router) + React 19 + TypeScript + Tailwind v4, backed by Supabase
...
outbound webhooks need `028_webhook_endpoints.sql`) — check `docs/` and
existing route comments for such dependencies before assuming a table/
column exists.
